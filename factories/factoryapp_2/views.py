...
from .serializers import SecondFactorySerializer
...
# 공장 pk를 필요로 하지않는 작업
class SecondFactoryAPIView(APIView):
    ...
    def get(self, request):
        try:
            factory_info = SecondFactory.objects.all()
            factories_serializer = serializers.SecondFactorySerializer(factory_info, many=True)
    ...
    @swagger_auto_schema(
        ...
        request_body=serializers.SecondFactorySerializer
    )
    def post(self, request):
        try:
            factory_serializer = serializers.SecondFactorySerializer(data=request.data, context={'request':request})
...
# 공장 pk를 필요로 하는 작업
class SecondFactorySelectAPIView(APIView):
    permissions_classes = (AllowAny,)
    parser_classes = (MultiPartParser,)
    ...
    def get_factory_object(self, pk):
        try:
            factory_info = SecondFactory.objects.get(id=pk)
            return factory_info
        except:
            return Response({'result': False}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def get(self, request, pk):
        try:
            factory_info = self.get_factory_object(pk=pk)
            factory_serializer = serializers.SecondFactorySerializer(factory_info)
    ...
    def patch(self, request, pk):
        try:
            ...
            if request.FILES.get("second_business_registration_file") and factory_info.second_business_registration_file:
                os.remove(os.path.join(settings.MEDIA_ROOT, factory_info.second_business_registration_file.path))
            
            factory_serializer = serializers.SecondFactorySerializer(instance=factory_info, data=request.data)
